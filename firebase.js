...
// Enable offline database persistence immediately on startup
enableMultiTabIndexedDbPersistence(db).catch((err) => {
  if (err.code === 'failed-precondition') {
    enableIndexedDbPersistence(db).catch((err2) => {
      console.warn("Firestore single-tab offline persistence failed:", err2);
    });
  } else {
    console.warn("Firestore multi-tab offline persistence failed:", err);
  }
});
