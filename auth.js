...
// Singular main Authentication handler with integrated instant load and cache resolution
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    try {
      ...
      let userData = null;
      try {
        // Attempt immediate load from Firestore persistent offline storage
        const cachedSnap = await getDocFromCache(doc(db, "users", user.uid));
        if (cachedSnap.exists()) {
          userData = cachedSnap.data();
        }
      } catch (cacheErr) {
        console.log("Profile details not in local cache; executing server query.");
      }
      ...
      // Override Login history with Arena Home state so the Back button cannot return to Login
      window._viewHistory = ['home'];
      window.history.replaceState({ app: "arena" }, "");
      window.history.pushState({ app: "arena" }, "");
      window._historyStatePushed = true;
    ...
    window._viewHistory = [];
    window._historyStatePushed = false;
